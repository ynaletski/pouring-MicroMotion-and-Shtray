...
int f_dlv_liq(int key)
{
   ...
   switch(sw_dlv_liq)
   {
      ...
      case  5:
         ...
         MmiGotoxy(0,4);    MmiPuts(list_avt[8]); //"Обнуление отсчета расходомера");
      ...
      case  6:
         ...
         {  // время вышло
            //01.10.20 YN -(.)-(.)- 
               //was:(0,1)
               MmiGotoxy(0,6);  MmiPuts(list_avt[7]); //"Таймаут связи с расходомером ");
            //-------- YN -//\\-
            f_icp_errS(MVD_com);
            goto m_err;
         }
          ...
          if(flag_prok == 0)
           {
             MVD_t_rslt[0]=0;
             time_t_snd=TimeStamp;
             if( f_MVD_WR((int)0,(int)MVD_WR_C,(int) 2,(int)1,(long int) 0,(float)0)== 0) //старт тоталайзер
             { // ошибка при отправке посылки MVD
                   goto m_err_mvd;
             }
           }
