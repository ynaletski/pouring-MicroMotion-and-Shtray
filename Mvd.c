...
//02.07.2021 YN -(.)-(.)-
  float newMassTotal = 0.0;
  int newFlagCount = 0;
  float newTimeNew = 0.0;
  float newTimeOld = 0.0;
//-------- YN -//\\-

//01.10.20 YN -(.)-(.)-
  struct s_mass_time s_frd=
  {
  0.0,  //float mass_old; //M1
  0.0,  //float mass_x;   //Mx
  0.0,  //float mass_new; //M2
  0,    //unsigned long t_old; //T1
  0,    //unsigned long t_x;   //Tx
  0,    //unsigned long t_new; //T2
  //13.10.20 YN -(.)-(.)-
...
//02.07.2021 YN -(.)-(.)-
  //39,75,  // unit for mass flow, 75-kg/hour
  39,70,  // unit for mass flow, 70-Grams/second
//-------- YN -//\\-

  40,92,  // Density unit kg/m3 --

//02.07.2021 YN -(.)-(.)-
  //45,61,  // unit for mass total,61- kg
  45,60,  // unit for mass total,60-g
//-------- YN -//\\-
...
// 0x1005+1,0x44,  // расход в обе стороны, т/ч, л/час
  ...
  //07.07.2022 YN -(.)-(.)-
    45,61,  // unit for mass total,60-g
    //45,60,  // unit for mass total,61- kg, 60-g       //Не применяется YN -(.)-(.)-
  //-------- YN -//\\-
...
//     MVD_fl[ii]=-1; // qqqqqq
  ...
  // проверяет текущее значение порядка
    ...
    case 2:
     ...
     //16.09.2022 YN if((MVD_fn[0] == F_FAULT) || (s_MVD[ii].i == 2) ) // ! 1 for Micromotion ( differ from Micromotion)  //закоментировать для новых расходомеров
       { // byte order is correct or RFT9739 device connected
         MVD_fl[ii]=3;
         ii_MVD_i=0;
         ii_MVD_fl=0;
      ...
      /*//16.09.2022 YN
       MVD_fn[ii]=MVD_WR_I;                                                         //закоментировать для новых расходомеров 
       s_MVD[ii].r_addr=521; // I521  Byte order code  for float registers          //закоментировать для новых расходомеров
       s_MVD[ii].i = 2;      // ! 1 for Micromotion                                 //закоментировать для новых расходомеров
       if((n_send[ii]++) > 3)                                                       //закоментировать для новых расходомеров
       {                                                                            //закоментировать для новых расходомеров
         flag_mvd_ton=0;                                                            //закоментировать для новых расходомеров
         f_icp_error(&MVD[ii],WR_ERR );                                             //закоментировать для новых расходомеров
         MVD_fl[ii]=-1;                                                             //закоментировать для новых расходомеров
       }                                                                            //закоментировать для новых расходомеров
       MVD_fl[ii]=1;                                                                //закоментировать для новых расходомеров
       break;                                                                       //закоментировать для новых расходомеров
       */
...
//-----------------------------------------
int ModeFill=F_Mass;
unsigned long tim_snd_MVD; //long int tim_snd_MVD;
long int lerr_MVD=0;
float VolI_beg=0;
...
void f_lin_intrpl()
{
    ...
    //02.07.2021 YN -(.)-(.)-
      if(newFlagCount == 1)
      {
        ...
        {
          newMassTotal += s_MVD[0].FlowM * (((float)s_back.t_x - back_T1) / 1000);
          newFlagCount = 0;
        }
        else
        {
          newTimeNew = (float)(tim_snd_MVD - start_time);
          if(newTimeNew > newTimeOld)
          {
            newMassTotal += s_MVD[0].FlowM * ((newTimeNew - newTimeOld) / 1000);
            newTimeOld = newTimeNew;
          }
        }  
      }
    //-------- YN -//\\-  
  ...
  if(State_SLV == en_start) //62 регистр I8 enable старт разрешен для перекидки
  {
    ...
    s_frd.mass_new = s_MVD[0].MassT;
              //02.07.2021 YN -(.)-(.)-
                if(flag_motion == fl_frd_x && newFlagCount == 0 && frd_T2 > (float)s_frd.t_x)
                {
                  newMassTotal = s_MVD[0].FlowM * ((frd_T2 - (float)s_frd.t_x) / 1000);
                  newFlagCount = 1;
                  newTimeOld = frd_T2;
                }
              //-------- YN -//\\-  
    ...
    // Проверяем условие: если время в момент сдвига перекидки больше чем T2, то T1 = T2
    if(flag_motion == fl_frd_x && frd_T2 > (float)s_frd.t_x) goto end;
  ...
  {
    back_T1 = (float) (tim_snd_MVD - start_time);
  ...
  else if(State_SLV == en_start_back) //64 регистр I8 enable старт разрешен для перекидки в обратном направлении
  {
    ...
    // Проверяем условие: если время в момент сдвига перекидки больше чем T2, то T1 = T2
    if(flag_motion == fl_back_x && back_T2 > (float)s_back.t_x) goto end;
...
{ //метод сличения
  if(State_SLV == en_cmpr_strt) //62 регистр I8 enable старт разрешен для измерения
  {
    ...
    // Проверяем условие: если время в момент сдвига перекидки больше чем T2, то T1 = T2
    if(flag_motion == fl_frd_x && frd_T2 > (float)s_frd.t_x) goto end1;
  ...
  else if(State_SLV == en_cmpr_cnt)
  {
    ...
    // Проверяем условие: если время в момент сдвига перекидки больше чем T2, то T1 = T2
    if(flag_motion == fl_back_x && back_T2 > (float)s_back.t_x) goto end1;
