...
&s_MVD[0].MassT,    //  0 F1000 Масса в текущей операции отпуска (total)   Mass total     ,I259
&s_MVD[0].FlowM,    //  1 F1002  Массовый расход, кг/ч                      Mass flow      ,I247
&s_MVD[0].Dens,     //  2 F1004 Плотность кг/м3                            Density        ,I249
&s_MVD[0].Temp,     //  3 F1006 Температура,                               Temperature    ,I251
//&s_MVD[0].VolI,     // 15 F1030 Объем инвентарный ( накапливающий счетчик) Volume invntory,I265

&s_MVD[0].VolT,     // 4 F1008 Объем в текущей операции отпуска (total)
&s_MVD[0].FlowV,    // 5 F1010 Объемный расход, л/ч

&s_frd.vol_x,        // 6 F1012 Объем при движении перекидки на весы (интерполированный)
&s_frd.mass_x,       // 7 F1014 Масса при движении перекидки на весы (интерполированая)

&frd_T1,             // 8 F1016 Время перед первым концевиком, (мс - на весы и сличение, мкс - измерение времени) 
&frd_Tx,             // 9 F1018 Время первого концевика, (мс - на весы и сличение, мкс - измерение времени)
&frd_T2,             // 10 F1020 Время после первого концевика, (мс - на весы и сличение, мкс - измерение времени)

&s_back.vol_x,       // 11 F1022 Объем при движении перекидки в бак (интерполированный)
&s_back.mass_x,      // 12 F1024 Масса при движении перекидки в бак (интерполированая)

&back_T1,             // 13 F1026 Время перед вторым концевиком, (мс - на весы и сличение, мкс - измерение времени)
&back_Tx,             // 14 F1028 Время второго концевика, (мс - на весы и сличение, мкс - измерение времени)
&back_T2,             // 15 F1030 Время после второго концевика, (мс - на весы и сличение, мкс - измерение времени)
      ...
      case ch_time:
        ...
        switch (State_SLV)
        {
          case bak:
            ...
            //05.04.2021 YN -(.)-(.)- 
              //на втором концевике без промежуточного состояния (ошибка)
       ...
       switch(*(int *)list_Slv_int[Addr] )
       {
        case 0:
          ...
          //01.10.20 YN -(.)-(.)-
            //added:
            sw_mmi=0;
            State_SLV = Cmd_brk; //0 РЕГИСТР I8
...
//30.03.2021 YN -(.)-(.)- 
  State_SLVf=(float) State_SLV;
  tolic.i[0]=CMD_IN;      //i7
  tolic.i[1]=State_SLV;   //i8
//           --//\\--
